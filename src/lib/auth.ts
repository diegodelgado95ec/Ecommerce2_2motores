...
class AuthService {
  ...
  async login(email: string, password: string): Promise<{ user: User; token: string }> {
    ...
    if (!user) {
      await this.logAccess(undefined, 'login_failed', 'user', false);
      throw new Error('Email o contraseña incorrectos');
    }

    // Verificar si está activo
    if (!user.isActive) {
      await this.logAccess(user.id, 'login_inactive', 'user', false);
    ...
    if (!isValidPassword) {
      await this.logAccess(user.id, 'login_failed', 'user', false);
      throw new Error('Email o contraseña incorrectos');
    }
    ...
    const expiresAt = new Date();
      ...
      const sessionId = await db.add('sessions', {
        userId: user.id!,
        token,
        expiresAt: expiresAt.toISOString(), // ✅ CORREGIDO
        createdAt: new Date().toISOString(), // ✅ CORREGIDO
      } as any);
    ...
    await this.logAccess(user.id, 'login_success', 'user', true);
